...
design principles as shown in [17]. An example CNN architecture is shown in Figure 1. \newline
...
results in [1], I implemented and trained adversarial networks on the MNIST dataset [18] $^1$. The two networks were simple MLP. Figure 2 shows some non-cherry
...
\begin{figure}[!htb]
   \begin{center}$
      ...
      \end{array}$
   \end{center}
   \caption{\textit{Results from training DCGANs on MNIST after only 3 epochs.}}
\end{figure}
...
the latent vector $z$. Figure 4 shows how modifications in the latent space affect the generated images. This is further explored in [13], which combines
cGANs with an auto-encoder to control the attributes contained in generated images in a more explicit manner. To compare with the results I obtained from [1], I implemented and trained a DCGAN on MNIST.
Results can be seen in Figure 3, and clearly contain much less noise$^1$.
...
\[ PT(B) = \frac{1}{N(N-1)} \sum_i{\sum_{j\neq i}{(\frac{S^T_iS_j}{||S_i||||S_j||})^2}}\]
...
Results can be seen in Figure 6, and visually match those presented in the paper \footnote{Code for EBGAN results can be found at: https://github.com/cameronfabbri/EBGAN-Tensorflow}.

\begin{figure}[!htb]
   \begin{center}$
      \begin{array}{cccc}
         \includegraphics[width=4in]{ebgan0_results}
      \end{array}$
   \end{center}
...
\begin{figure}[!htb]
   \begin{center}$
      \begin{array}{cccc}
         \includegraphics[width=4in]{wgan_results}
      \end{array}$
   \end{center}
   \caption{\textit{Results from training WGAN on faces from the CelebA dataset.}}
\end{figure}

\noindent One very nice property of the WGAN that is not seen in the other GAN variants mentioned is a meaningful loss function that directly correlates with image quality. Figure 8 shows
the generator and discriminator losses for the DCGAN trained for generating the MNIST images in Figure 3 compared with the generator and critic losses for the WGAN trained for CelebaA results
in Figure 7. While visual quality improves in both models, only the WGAN losses appear to converge, while the DCGAN losses remain stable, making it impossible to gauge how well the model is performing.
...
\begin{figure}[!htb]
   \begin{center}$
      \begin{array}{cccc}
         \includegraphics[width=3in]{dcgan_errG}
         \includegraphics[width=3in]{dcgan_errD} \\
         \includegraphics[width=3in]{wgan_errG}
         \includegraphics[width=3in]{wgan_errD}
      \end{array}$
   \end{center}
...
\noindent I adopted their model architecture for the task of colorizing grayscale images, and experimented with the several variations of GANs discussed earlier. Along with combining $L_1$, I also
experimented with $L_2$ as well. I used the $LAB$ colorspace in place of $RGB$, as the Euclidean distances between colors better model that of human vision. The input to the generator is the lightness channel $L$,
...
https://github.com/cameronfabbri/Colorizing-Images-Using-Adversarial-Networks}. Most surprising was the EBGAN not being able to generate hardly any color output. My thoughts are that the autoencoder was not strong enough,
and therefore upon reconstruction the reconstructed fake images and reconstructed real images were too similar visually. WGAN variations were not as thoroughly explored due to the extremely long training time.
The other GAN variations took a little over 2 days to train on the 200,000 images from CelebA on a GTX 1080 (8 epochs). The original GAN performed well, although LSGANs gave the best performance. I believe this is due to
the fact that the discriminator has a very difficult job due to the structural similarity between the real and generated images in comparison to the classical adversarial setup of testing the discriminator against images
generated from noise. In LSGANs, even though the discriminator may be fooled, the image may not be close to the true data distribution, which the loss function takes account for. \newline

\begin{figure}[!htb]
   \begin{center}$
      \begin{array}{cccc}
      \includegraphics[width=0.75in]{1_gray}
      \includegraphics[width=0.75in]{1_gan_100_0_col}
      \includegraphics[width=0.75in]{1_gan_0_1_col}
      %\includegraphics[width=0.75in]{1_lsgan_100_0_col}
      \includegraphics[width=0.75in]{1_lsgan_0_1_col}
      %\includegraphics[width=0.75in]{1_ebgan_100_0_col}
      %\includegraphics[width=0.75in]{1_ebgan_0_1_col}
      %\includegraphics[width=0.75in]{1_wgan_100_0_col}
      %\includegraphics[width=0.75in]{1_wgan_0_1_col} \\
      \includegraphics[width=0.75in]{1_true}
      \\
      \includegraphics[width=0.75in]{2_gray}
      \includegraphics[width=0.75in]{2_gan_100_0_col}
      \includegraphics[width=0.75in]{2_gan_0_1_col}
      %\includegraphics[width=0.75in]{2_lsgan_100_0_col}
      \includegraphics[width=0.75in]{2_lsgan_0_1_col}
      %\includegraphics[width=0.75in]{2_ebgan_100_0_col}
      %\includegraphics[width=0.75in]{2_ebgan_0_1_col}
      %\includegraphics[width=0.75in]{2_wgan_100_0_col}
      %\includegraphics[width=0.75in]{2_wgan_0_1_col} \\
      \includegraphics[width=0.75in]{2_true}
      \\
      \includegraphics[width=0.75in]{3_gray}
      \includegraphics[width=0.75in]{3_gan_100_0_col}
      \includegraphics[width=0.75in]{3_gan_0_1_col}
      %\includegraphics[width=0.75in]{3_lsgan_100_0_col}
      \includegraphics[width=0.75in]{3_lsgan_0_1_col}
      %\includegraphics[width=0.75in]{3_ebgan_100_0_col}
      %\includegraphics[width=0.75in]{3_ebgan_0_1_col}
      %\includegraphics[width=0.75in]{3_wgan_100_0_col}
      %\includegraphics[width=0.75in]{3_wgan_0_1_col} \\
      \includegraphics[width=0.75in]{3_true}
      \\
      \includegraphics[width=0.75in]{4_gray}
      \includegraphics[width=0.75in]{4_gan_100_0_col}
      \includegraphics[width=0.75in]{4_gan_0_1_col}
      %\includegraphics[width=0.75in]{4_lsgan_100_0_col}
      \includegraphics[width=0.75in]{4_lsgan_0_1_col}
      %\includegraphics[width=0.75in]{4_ebgan_100_0_col}
      %\includegraphics[width=0.75in]{4_ebgan_0_1_col}
      %\includegraphics[width=0.75in]{4_wgan_100_0_col}
      %\includegraphics[width=0.75in]{4_wgan_0_1_col} \\
      \includegraphics[width=0.75in]{4_true}
...
\noindent It is important to note that because these are not "true" grayscale images, what is being learned is the mapping from the function used to convert these images to grayscale back to color. To
compare, I also tested on a true grayscale image of John F. Kennedy, seen in Figure 10.

\begin{figure}[!htb]
   \begin{center}$
      \begin{array}{cccc}
         \includegraphics[width=1in]{gray_jfk} % TODO need to make these all consistant with L1 and L2 weights
         \includegraphics[width=1in]{col_jfk_gan_0_1_1}
         \includegraphics[width=1in]{col_jfk_ls_0_1_1}
         \includegraphics[width=1in]{col_jfk_e_100_0_1}
         \includegraphics[width=1in]{col_jfk_w_100_0_1}
      \end{array}$
   \end{center}
   \caption{Colorizing a true black and white photo. From left to right: Input image, GAN, LSGAN, EBGAN, WGAN}
\end{figure}
...
any structure similar to MNIST. Future work involves exploring these variations, as well as tackling the multiclass colorization problem.
