...
results in [1], I implemented and trained adversarial networks on the MNIST dataset$^1$. The two networks were simple MLP. Figure 2 shows some non-cherry
...
\begin{figure}[!htb]
   \begin{center}$
      ...
      \end{array}$
   \end{center}
   \caption{Results from training LSGANs on churches from the LSUN dataset. Note these are 128x128 images.}
\end{figure}
...
\begin{figure}[!htb]
   \begin{center}$
      ...
      \end{array}$
   \end{center}
   \caption{Results from training EBGAN on faces from the CelebA dataset.}
\end{figure}
...
\noindent Not surprisingly, such a function $f_w$ can be found by training a neural network with weights $w \in W$. To ensure this function is $K$-Lipshitz, the weights are clamped to some range.
...
\begin{figure}[!htb]
   \begin{center}$
      ...
      \end{array}$
   \end{center}
   \caption{Results from training WGAN on faces from the CelebA dataset.}
\end{figure}

\noindent One very nice property of the WGAN that is not seen in the other GAN varients mentioned is a meaningful loss function that directly correlates with image quality. Figure 8 shows
...
generated images (seperately). I experimented on the CelebA dataset, as well as the multiclass STL-10 dataset [17]. Results can be seen in Figures 9 and 10. Most surprising was the EBGAN not being
...
too similar visually. WGAN variations were not as thoroughly explored due to the extremely long training time. The other GAN variations took a little over 2 days to train the 200,000 images from CelebA
on a GTX 1080. The original GAN performed well, although LSGANs gave the best performance. I believe this is due to the fact that the discriminator has a very difficult job due to the structural similarity
...
fooled, the image may not be close to the true data distribution, which the loss function takes account for. \textit{More here as well as some figures with colorizations}
