...
that may have fooled the discriminator but do not lie close to the true data distribution.
The objective functions for $D$ and $G$ are given as:

%\[ \mathcal{L}_{DCGAN}(G,D) = \mathbb{E}_{x,y \sim p_{data}} log D(x,y) + \mathbb{E}_{x \sim p_{data}}
\[\mathcal{L}_{LSGAN}(G,D) = \frac{1}{2} \mathbb{E}_{x \sim p_{data(x)}} [(D(\textbf{\textit{x}})-b)^2] +
\frac{1}{2} \mathbb{E}_{z \sim p_z(z)}[(D(G(z)) - a)^2] \]

\[ \frac{1}{2} \mathbb{E}_{x \sim p_{data(x)}} [(D(G(\textbf{\textit{x}}))-c)^2] \]

\noindent where $p_{data}$ represents the true data, $p_z$ represents some noise prior, e.g.
$z \sim \mathcal{N}$, $a=0$ to denote the fake data, $b=1$ to denote the true data,
and $c=1$ in order to try and fool $D$.

We combine the LSGAN objective with $L_1$ and $L_2$ in order to provide some sense of ground truth. These can
be expressed as:

\[ \mathcal{L}_{LSGAN}(G,D) = \mathbb{E}_{x,y \sim p_{data}} log D(x,y) + \mathbb{E}_{x \sim p_{data}}
log (1-D(x, G(x)))\]
...
%\[ \mathcal{L}_D(x_{color}, x_{gray}) = -(D(x_{color}) + D(G(x_{gray})))\]
%\[ \mathcal{L}_G(x_{gray}) = -(log(D(G(x_{gray})))) + \lambda_1 L_1 + \lambda_2 L_2 \]
