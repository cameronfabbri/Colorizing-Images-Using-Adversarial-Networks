...
We found that this model performs better with $L_1$ loss function compared to $L_2$ loss. 
Results on few test cases are shown in Fig. \ref{fig:res_full} (a)-(b), which demonstrate decent colorization performance of this model. 
...
Once trained only a few epochs, it seems to predict the colors for face, eyes, and hair very well. 
It has difficulty colorizing the random background and some averaging effect is there; however, the overall performance is decent enough to fool a human. On the other hand, for Places2 dataset, we observed that it can colorize objects with monotonous colors (such as grass, roads, etc.) very well. However, it has difficulty colorizing opaque objects (such as glass, sky with clouds, etc.).
