...
if __name__ == '__main__':
   ...
   if LOSS_METHOD == 'energy':
      ...
      D_img_real = pix2pix.netD(L_image, ab_image, NUM_GPU)
      D_img_fake = pix2pix.netD(L_image, gen_ab, NUM_GPU, reuse=True)
      
      D_ab_real  = pix2pix.netD_ab(ab_image, NUM_GPU)
      D_ab_fake  = pix2pix.netD_ab(gen_ab, NUM_GPU, reuse=True)
   
   e = 1e-12
   if LOSS_METHOD == 'wasserstein':
      print 'Using Wasserstein loss'
      D_img_real = lrelu(D_img_real)
      D_img_fake = lrelu(D_img_fake)
      D_ab_real = lrelu(D_ab_real)
      D_ab_fake = lrelu(D_ab_fake)
      
      D_real = D_img_real + D_ab_real
      D_fake = D_img_fake + D_ab_fake
      ...
      if L1_WEIGHT <= 0.0 and L2_WEIGHT <= 0.0:
         ...
         errG = gen_loss_GAN
      errD1 = tf.reduce_mean(D_img_real - D_img_fake)
      errD2 = tf.reduce_mean(D_ab_real  - D_ab_fake)
      errD = errD1 + errD2

   if LOSS_METHOD == 'least_squares':
      print 'Using least squares loss'
      # Least squares requires sigmoid activation on D
      errD_img_real = tf.nn.sigmoid(D_img_real)
      errD_img_fake = tf.nn.sigmoid(D_img_fake)
      errD_ab_real  = tf.nn.sigmoid(D_ab_real)
      errD_ab_fake  = tf.nn.sigmoid(D_ab_fake)
         
      errD_real = D_img_real + D_ab_real
      errD_fake = D_img_fake + D_ab_fake
      ...
      errD1 = tf.reduce_mean(0.5*(tf.square(errD_img_real - 1)) + 0.5*(tf.square(errD_img_fake)))
      errD2 = tf.reduce_mean(0.5*(tf.square(errD_ab_real - 1)) + 0.5*(tf.square(errD_ab_fake)))
      errD = errD1 + errD2

   if LOSS_METHOD == 'gan' or LOSS_METHOD == 'cnn':
      print 'Using original GAN loss'
      if LOSS_METHOD is not 'cnn':
         D_img_real = tf.nn.sigmoid(D_img_real)
         D_img_fake = tf.nn.sigmoid(D_img_fake)
         D_ab_real = tf.nn.sigmoid(D_ab_real)
         D_ab_fake = tf.nn.sigmoid(D_ab_fake)
      ...
      if L1_WEIGHT <= 0.0 and L2_WEIGHT <= 0.0:
         ...
         #errD = errD + e
         errG = gen_loss_GAN
      errD1 = tf.reduce_mean(-(tf.log(D_real+e)+tf.log(1-D_fake+e)))
      errD2 = tf.reduce_mean(-(tf.log(D_real+e)+tf.log(1-D_fake+e)))
